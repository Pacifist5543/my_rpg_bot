    ...
    *_, direction_elf = callback.data.split("_")
    ...
    if user.race == "Эльф":
        ...
        if direction_elf == 'razvilka':
            ...
            continue_kb.row(
                types.InlineKeyboardButton("Налево", callback_data="continue_adventure_left"),
                types.InlineKeyboardButton("Направо", callback_data="continue_adventure_right")
            )
        ...
        # Левый путь
            ...
            continue_kb.row(
                types.InlineKeyboardButton("Да", callback_data="continue_adventure_holmup"),
                types.InlineKeyboardButton("Нет", callback_data="continue_adventure_notholm")
            )
        ...
        # Подъем на холм
            ...
            user.defence += 0.15
            session.commit()
            continue_kb.add(types.InlineKeyboardButton("Продолжить путь", callback_data="continue_adventure_afterhill"))
        
        # Обход холма
        elif direction_elf == 'notholm':
            story = 'Ты обошел холм стороной и продолжил путь через лес.'
            continue_kb.add(types.InlineKeyboardButton("Идти дальше", callback_data="continue_adventure_afterhill"))
        
        # Правый путь
        elif direction_elf == 'right':
            story = 'Ты пошел направо...\nи встретил разбойников!'
            continue_kb.row(
                types.InlineKeyboardButton("Сражаться", callback_data="continue_adventure_fight"),
                types.InlineKeyboardButton("Убежать", callback_data="continue_adventure_run")
            )
        ...
        # Бой с разбойниками
            ...
            if user.damage >= 0.15:
                ...
                user.damage += 0.5  # Награда за победу
                session.commit()
                continue_kb.add(types.InlineKeyboardButton("Следовать по карте", callback_data="continue_adventure_treasure"))
            else:
                story = 'Разбойники оказались слишком сильными! Ты едва спасся.\nВернуться на развилку?'
                continue_kb.add(types.InlineKeyboardButton("Вернуться", callback_data="continue_adventure_razvilka"))
        
        # Убегание от разбойников
        elif direction_elf == 'run':
            story = 'Ты успешно убежал и вернулся на развилку.'
            continue_kb.add(types.InlineKeyboardButton("Выбрать путь снова", callback_data="continue_adventure_razvilka"))
        ...
        # Находка сокровища
            ...
            user.defence += 0.10
            session.commit()
            continue_kb.add(types.InlineKeyboardButton("Идти к финальной битве", callback_data="continue_adventure_finalfight"))
        
        # Путь после холма
        elif direction_elf == 'afterhill':
            story = 'Ты вышел на поляну, где стоит поджигатель леса!\nОн виновен в гибели твоего дома.'
            continue_kb.add(types.InlineKeyboardButton("Атаковать", callback_data="continue_adventure_finalfight"))
        
        # Финальный бой
        elif direction_elf == 'finalfight':
            if user.damage >= 20 and user.defence >= 0.25:
                story = 'После долгого боя ты побеждаешь поджигателя!\nОн умоляет о пощаде...'
                continue_kb.row(
                    types.InlineKeyboardButton("Пощадить", callback_data="elf_end_mercy"),
                ...
                continue_kb.add(types.InlineKeyboardButton("Попробовать снова", callback_data="continue_adventure_afterhill"))
...
@bot.callback_query_handler(func=lambda callback: callback.data.startswith("human_adventure"))
    ...
    *_, direction_people = callback.data.split("_")
    ...
    if user.race == "Человек":
        ...
        if direction_people == "start":
            story = ("Ты - обычный человек в королевстве Альмерия. "
                    "Сегодня утром в твою деревню напали бандиты.\n\n"
                    "Что будешь делать?")
            continue_kb.row(
                types.InlineKeyboardButton("Сражаться", callback_data="human_adventure_fight"),
                types.InlineKeyboardButton("Бежать в город", callback_data="human_adventure_run")
            )
        
        # Вариант 1: Сражение с бандитами
        elif direction_people == "fight":
            if user.damage >= 0.10:
                story = ("Ты героически сражался и отбил атаку!\n"
                        "Деревенский староста даёт тебе меч (+5 к урону).\n"
                        "Теперь нужно решить что делать дальше.")
                user.damage += 0.5
                session.commit()
                continue_kb.row(
                    types.InlineKeyboardButton("Идти в город за помощью", callback_data="human_adventure_city"),
                    types.InlineKeyboardButton("Преследовать бандитов", callback_data="human_adventure_chase")
                )
            else:
                story = ("Бандиты оказались сильнее... Ты тяжело ранен.\n"
                        "Придётся отступить в город за помощью.")
                user.health -= 0.20
                session.commit()
                continue_kb.add(types.InlineKeyboardButton("Идти в город", callback_data="human_adventure_city"))
        
        # Вариант 2: Бегство в город
        elif direction_people == "run":
            story = ("Ты успешно добрался до города.\n"
                    "В таверне ты слышишь разговоры о набегах бандитов.\n"
                    "Что будешь делать?")
            continue_kb.row(
                types.InlineKeyboardButton("Наняться в городскую стражу", callback_data="human_adventure_guard"),
                types.InlineKeyboardButton("Искать информацию", callback_data="human_adventure_info")
            )
        ...
        # Развилка в городе
            ...
            story = ("Ты прибыл в город. Здесь кипит жизнь.\n"
                    "Куда отправишься?")
            continue_kb.row(
                types.InlineKeyboardButton("В таверну", callback_data="human_adventure_tavern"),
                types.InlineKeyboardButton("К замку", callback_data="human_adventure_castle"),
                types.InlineKeyboardButton("На рынок", callback_data="human_adventure_market")
            )
        
        # Преследование бандитов
        elif direction_people == "chase":
            story = ("Ты выследил бандитов до их лагеря.\n"
                    "Видишь их главаря и пленных из деревни.\n"
                    "Как поступишь?")
            continue_kb.row(
                types.InlineKeyboardButton("Атаковать сразу", callback_data="human_adventure_attack"),
                types.InlineKeyboardButton("Подождать ночи", callback_data="human_adventure_night"),
                types.InlineKeyboardButton("Вернуться за помощью", callback_data="human_adventure_city")
            )
        
        # Атака на лагерь
        elif direction_people == "attack":
            if user.damage >= 0.15:
                story = ("Ты побеждаешь бандитов и освобождаешь пленных!\n"
                        "Жители дарят тебе кольчугу (+10 к защите).\n"
                        "Теперь ты местный герой!")
                user.defence += 0.10
                session.commit()
                continue_kb.add(types.InlineKeyboardButton("Закончить приключение", callback_data="human_adventure_goodend"))
            else:
                story = ("Бандитов оказалось слишком много...\n"
                        "Тебя берут в плен. Попробовать сбежать?")
                continue_kb.row(
                    types.InlineKeyboardButton("Попытаться сбежать", callback_data="human_adventure_escape"),
                    types.InlineKeyboardButton("Ждать помощи", callback_data="human_adventure_wait")
                )
        
        # Ночная атака
        elif direction_people == "night":
            story = ("Ночью ты незаметно проникаешь в лагерь.\n"
                    "Тебе удаётся освободить пленных!\n"
                    "Что будешь делать дальше?")
            continue_kb.row(
                types.InlineKeyboardButton("Поджечь лагерь", callback_data="human_adventure_fire"),
                types.InlineKeyboardButton("Тихо уйти", callback_data="human_adventure_leave")
            )
        
        # Работа в страже
        elif direction_people == "guard":
            story = ("Ты поступаешь на службу в городскую стражу.\n"
                    "После месяца тренировок твои навыки улучшились!\n"
                    "+7 к урону, +5 к защите")
            user.damage += 0.7
            user.defence += 0.5
            session.commit()
            continue_kb.add(types.InlineKeyboardButton("Отправиться на задание", callback_data="human_adventure_mission"))
        
        # Поиск информации
        elif direction_people == "info":
            story = ("Ты узнаёшь, что бандиты работают на барона Крега.\n"
                    "Он хочет свергнуть короля!\n"
                    "Что будешь делать?")
            continue_kb.row(
                types.InlineKeyboardButton("Доложить властям", callback_data="human_adventure_report"),
                types.InlineKeyboardButton("Расследовать самому", callback_data="human_adventure_investigate")
            )
        
        # Хорошая концовка
        elif direction_people == "goodend":
            story = ("Ты стал героем королевства!\n"
        ...
        # Плохая концовка (плен)
            ...
            story = ("Бандиты продали тебя в рабство...\n\n"
        ...
        # Обработка неверных путей
            ...
            continue_kb.add(types.InlineKeyboardButton("В город", callback_data="human_adventure_city"))
...
@bot.callback_query_handler(func=lambda callback: callback.data.startswith("skeleton_adventure"))
    ...
    *_, direction_skelet = callback.data.split("_")
    ...
    if user.race == "Скелет":
        ...
        if direction_skelet == "awaken":
            story = ("Ты пробуждаешься в древней гробнице. Кости твоего тела скрипят, а в глазницах мерцают \n"
            ...
            continue_kb.row(
                types.InlineKeyboardButton("Прямо", callback_data="skeleton_adventure_forward"),
                types.InlineKeyboardButton("Налево", callback_data="skeleton_adventure_left"),
                types.InlineKeyboardButton("Направо", callback_data="skeleton_adventure_right")
            )
        
        # Путь прямо
        elif direction_skelet == "forward":
            story = ("Ты находишь древний алтарь некроманта. На нём лежит:\n"
                    "1️⃣ Проклятый меч (+0.3 к урону, но -0.1 к защите)\n"
                    "2️⃣ Книга заклинаний (+0.2 к магии)\n"
                    "3️⃣ Пройти мимо")
            continue_kb.row(
                types.InlineKeyboardButton("Взять меч", callback_data="skeleton_adventure_cursed_sword"),
                types.InlineKeyboardButton("Взять книгу", callback_data="skeleton_adventure_spellbook")
            )
            continue_kb.add(types.InlineKeyboardButton("Пройти мимо", callback_data="skeleton_adventure_ignore_altar"))
        
        # Взятие проклятого меча
        elif direction_skelet == "cursed_sword":
            story = ("Ты взял меч, и почувствовал как твои кости наполняются силой!\n"
                    "Но защита немного ослабла...")
            user.damage += 0.3
            user.defence -= 0.1
            session.commit()
            continue_kb.add(types.InlineKeyboardButton("Идти дальше", callback_data="skeleton_adventure_deep_tomb"))
        
        # Взятие книги заклинаний
        elif direction_skelet == "spellbook":
            story = ("Ты изучил древние заклинания некроманта!\n"
        ...
        # Путь налево
            ...
            story = ("Ты находишь комнату с боевыми скелетами!\n"
                    "Они принимают тебя за своего командира.\n"
                    "Что будешь делать?")
            continue_kb.row(
                types.InlineKeyboardButton("Возглавить отряд", callback_data="skeleton_adventure_lead"),
                types.InlineKeyboardButton("Атаковать их", callback_data="skeleton_adventure_fight"),
                types.InlineKeyboardButton("Уйти", callback_data="skeleton_adventure_awaken")
            )
        
        # Возглавление отряда
        elif direction_skelet == "lead":
            story = ("Скелеты признают твоё превосходство!\n"
                    "Теперь у тебя есть отряд из 3 скелетов.\n"
                    "+0.5 к общей силе отряда")
            user.damage += 0.5
            session.commit()
            continue_kb.add(types.InlineKeyboardButton("Идти к выходу", callback_data="skeleton_adventure_exit"))
        ...
        # Бой со скелетами
            ...
            continue_kb.add(types.InlineKeyboardButton("Вернуться", callback_data="skeleton_adventure_awaken"))
        
        # Путь направо
        elif direction_skelet == "right":
            story = ("Ты чувствуешь свежий воздух! Это выход...\n"
                    "Но его охраняет гигантский скелет-страж.\n"
                    "Как поступишь?")
            continue_kb.row(
                types.InlineKeyboardButton("Сразиться", callback_data="skeleton_adventure_guardian"),
                types.InlineKeyboardButton("Попытаться обмануть", callback_data="skeleton_adventure_trick"),
                types.InlineKeyboardButton("Отступить", callback_data="skeleton_adventure_awaken")
            )
        
        # Бой со стражем
        elif direction_skelet == "guardian":
            if user.damage >= 0.6:
                story = ("Ты побеждаешь стража и выходишь на свободу!\n"
                        "На солнце твои кости начинают светиться...")
                continue_kb.add(types.InlineKeyboardButton("Завершить приключение", callback_data="skeleton_adventure_sun_end"))
            else:
        ...
        # Обман стража
            ...
            story = ("Ты притворяешься слугой Некроманта и проходишь!\n"
                    "Теперь ты на свободе, но кто-то ищет самозванца...")
            continue_kb.add(types.InlineKeyboardButton("Продолжить", callback_data="skeleton_adventure_hide"))
        
        # Глубокие катакомбы
        elif direction_skelet == "deep_tomb":
            story = ("Ты попадаешь в главный зал гробницы.\n"
                    "Перед тобой саркофаг Древнего Короля.\n"
                    "Что будешь делать?")
            continue_kb.row(
                types.InlineKeyboardButton("Открыть", callback_data="skeleton_adventure_open_sarcophagus"),
                types.InlineKeyboardButton("Обыскать комнату", callback_data="skeleton_adventure_search"),
                types.InlineKeyboardButton("Уйти", callback_data="skeleton_adventure_exit")
            )
        ...
        # Открытие саркофага
            ...
            continue_kb.row(
                types.InlineKeyboardButton("Стать его генералом", callback_data="skeleton_adventure_general"),
                types.InlineKeyboardButton("Украсть его меч", callback_data="skeleton_adventure_steal"),
                types.InlineKeyboardButton("Отказаться", callback_data="skeleton_adventure_refuse")
            )
        
        # Концовки
        elif direction_skelet == "sun_end":
            story = ("Твои кости под солнечным светом превращаются в светящийся кристалл!\n\n"
                    "💀 Ты становишься Легендарным Светящимся Скелетом!")
            continue_kb = None
            
        elif direction_skelet == "general":
            story = ("Ты возглавляешь армию нежити!\n\n"
                    "👑 Ты стал Генералом Костяного Легиона!")
            continue_kb = None
            
        elif direction_skelet == "steal":
            if user.damage >= 0.7:
                story = ("Ты успешно крадёшь меч и сбегаешь!\n\n"
        ...
        # Обработка неверных путей
            ...
            continue_kb.add(types.InlineKeyboardButton("В начало", callback_data="skeleton_adventure_awaken"))
...
@bot.callback_query_handler(func=lambda callback: callback.data.startswith("werewolf_adventure"))
def handle_werewolf(callback: types.CallbackQuery):
    *_, direction_wervolf = callback.data.split("_")
    ...
    if user.race != "Вервольф":
            bot.answer_callback_query(callback.id, "Эта история не для твоей расы!", show_alert=True)
            return
...
Куда направишься?"""
            kb.row(
                types.InlineKeyboardButton("🏚️ К заброшенной деревне", callback_data="werewolf_village"),
                types.InlineKeyboardButton("🌳 Вглубь леса", callback_data="werewolf_forest"),
            )
            kb.add(types.InlineKeyboardButton("💧 К реке смыть кровь", callback_data="werewolf_river"))
...
3) Травяной аромат из избы 🧙"""
            ...
            kb.row(
                ...
                types.InlineKeyboardButton("👃 Обнюхать", callback_data="werewolf_sniff_pig"),
            )

    elif direction_wervolf == "kill_pig":
            user.damage += 0.2
            msg = "*Разрываешь добычу когтями!* Но шум привлёк внимание."
            kb.add(types.InlineKeyboardButton("⚠️ Приготовиться", callback_data="werewolf_alert"))
...
3) Древний дуб 🌳"""
            ...
            if user.animalism < 0.3:
                ...
                kb.add(types.InlineKeyboardButton("😾 Уйти", callback_data="werewolf_forest"))
            else:
                msg = "Волки признают в тебе сородича! +0.4 к контролю формы"
                user.animalism += 0.4
                kb.add(types.InlineKeyboardButton("🐕 Следовать за стаей", callback_data="werewolf_den"))
...
3) Рыбацкую лодку 🚣"""
            kb.row(
                types.InlineKeyboardButton("🌊 Отражение", callback_data="werewolf_reflection"),
        ...
        # ====== ОБРАБОТКА ОШИБОК ======
            ...
            kb.add(types.InlineKeyboardButton("🔙 Начать заново", callback_data="werewolf_start"))
...
@bot.callback_query_handler(func=lambda callback: callback.data.startswith("vampire_adventure"))
def handle_werewolf(callback: types.CallbackQuery):
...
+{boost.blood} к запасу крови"""
            kb.add(types.InlineKeyboardButton("🌆 Продолжить", callback_data="vampire_night"))
...
+{boost.health} здоровья"""
            kb.add(types.InlineKeyboardButton("💃 Продолжить", callback_data="vampire_dance"))
        ...
        # ====== ВЕТКА ЗАЩИТЫ ======
            ...
            kb.row(
                types.InlineKeyboardButton("🐺 Оборотни", callback_data="vampire_werewolves"),
...
{boost.health} к здоровью"""
            kb.add(types.InlineKeyboardButton("🏡 Вернуться", callback_data="vampire_awaken"))
...
+{boost.defense*100}% к защите"""
            kb.add(types.InlineKeyboardButton("🦇 Использовать", callback_data="vampire_power"))
...
+{boost.defense*100}% к защите"""
        ...
        # ====== ОБРАБОТКА ОШИБОК ======
            ...
            kb.add(types.InlineKeyboardButton("🔄 Начать заново", callback_data="vampire_awaken"))
    ...
    try:
            ...
            kb.add(types.InlineKeyboardButton("🔄 Перезапустить", callback_data="vampire_awaken"))
